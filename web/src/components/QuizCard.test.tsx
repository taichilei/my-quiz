...
describe('QuizCard Component', () => {
  ...
  const renderComponent = (questions: any[]) => {
    return render(
      <BrowserRouter>
        <QuizCard questions={questions} onFinish={mockOnFinish} />
      </BrowserRouter>
    )
  }
  ...
  describe('Give Up Feature', () => {
    it('should show explanation when giving up', () => {
      ...
      expect(screen.getByText((content) => content.includes('解析'))).toBeInTheDocument()
    })
  })
