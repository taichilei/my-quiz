...
import { questionApi, recordApi, examApi } from '../api/client'
...
describe('api/client.ts', () => {
  ...
  describe('questionApi', () => {
    ...
    it('get should fetch single question', async () => {
      const mockQuestion = { id: '1', type: 'single', content: 'Q1', answer: 'A' }
    ...
    it('create should POST question', async () => {
      const newQuestion = { type: 'single', content: 'New Q', answer: 'A' }
      const createdQuestion = { id: 'new123', ...newQuestion }
      ...
      const result = await questionApi.create(newQuestion as any)
    ...
    it('update should PUT question', async () => {
      ...
      await questionApi.update('1', updates as any)
  ...
  describe('examApi', () => {
    it('list should fetch all exams', async () => {
      const mockExams = [
        { name: '江苏省考', year: 2021, subject: '综合', part: '客观题', count: 100 },
      ]
  ...
  describe('recordApi', () => {
    it('create should POST record', async () => {
      ...
      global.fetch = vi.fn().mockResolvedValue({
        ok: true,
        json: () => Promise.resolve({ id: 'record123', ...newRecord }),
      })

      const result = await recordApi.create(newRecord as any)
    ...
    it('list should fetch user records', async () => {
      const mockRecords = [
        { id: '1', userId: 'user_123', questionId: 'q_1', isCorrect: true },
        { id: '2', userId: 'user_123', questionId: 'q_2', isCorrect: false },
      ]
    ...
    it('list should accept question filter', async () => {
      ...
      await recordApi.list('user_123', 'q_456')

      expect(global.fetch).toHaveBeenCalledWith(
        expect.stringContaining('questionId=q_456'),
        expect.any(Object)
      )
    })
